...
public:
	...
	std::string launch(const std::string& appId, const std::string& params);

	bool registerApplication(const std::string& appId, LSMessage *message);
	...
	const std::set<const LaunchPoint*>& enabledDockModeLaunchPoints() const { return m_dockModeLaunchPoints; }
...
private:
	...
	void focusApplication(const std::string& appId);
